...
#define TRY(expr) if(R_FAILED(res = (expr))) goto end;
    ...
    Draw_Lock();
    Draw_DrawString(10, 10, COLOR_TITLE, "Vidage memoire");
    const char * wait_message = "Veuillez patienter, cela peut prendre un certain temps...";
    ...
    {
        Draw_Lock();
        Draw_DrawString(10, 10, COLOR_TITLE, "Vidage m\x82""moire");
        Draw_DrawFormattedString(10, 30, COLOR_WHITE, "%*s", strlen(wait_message), " ");
        if(R_FAILED(res))
        {
            Draw_DrawFormattedString(10, 30, COLOR_WHITE, "Echec de l'op\x82""ration (0x%.8lx).", res);
        }
        else
        {
            Draw_DrawString(10, 30, COLOR_WHITE, "Operation reussie.");
        }
        Draw_DrawString(10, 30+SPACING_Y, COLOR_WHITE, "Appuyer sur B pour revenir en arri\x8A""re.");
...
static void ProcessListMenu_MemoryViewer(const ProcessInfo *info)
{
    ...
    if(R_SUCCEEDED(res))
    {
        ...
        if(codeAvailable || heapAvailable)
        {
            ...
            {
                Draw_Lock();
                Draw_DrawString(10, 10, COLOR_TITLE, "Visualiseur m\x82""moire");

                // Instructions
                const u32 instructionsY = 30;
                u32 viewerY = instructionsY + SPACING_Y + 6;
                Draw_DrawString(10, instructionsY, COLOR_WHITE, "D-PAD pour se d\x82""placer, X pour sauter, Y pour rechercher, A pour \x82""diter.");

                switch(menuMode)
                {
                    case MENU_MODE_NORMAL:
                        Draw_DrawString(10 + SPACING_X * 9, instructionsY, COLOR_GREEN, "d\x82""placer");
                        break;
                    case MENU_MODE_GOTO:
                        Draw_DrawString(10 + SPACING_X * 20, instructionsY, COLOR_GREEN, "sauter");
                        break;
                    case MENU_MODE_SEARCH:
                        Draw_DrawString(10 + SPACING_X * 31, instructionsY, COLOR_GREEN, "recherche");
                        break;
                    default: break;
                }
                ...
                if(codeAvailable && heapAvailable)
                {
                    Draw_DrawString(10, infoY, COLOR_WHITE, "Appuyez sur L ou R pour basculer entre le tas et le code..");
                    if((u32)menus[MENU_MODE_NORMAL].buf == heapDestAddress)
                        Draw_DrawString(10 + SPACING_X * 31, infoY, COLOR_GREEN, "tas");
                    if((u32)menus[MENU_MODE_NORMAL].buf == codeDestAddress)
                        Draw_DrawString(10 + SPACING_X * 40, infoY, COLOR_GREEN, "code");
                }
                else
                {
                    Draw_DrawString(10, infoY, COLOR_WHITE, "SELECT pour vider la m\x82""moire, START pour passer \x85"" l'affichage ASCII.");
                    if(ascii)
